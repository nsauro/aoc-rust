...
fn main() {
    ...
    fn try_parse_instruction<'a>(
        r: &Regex,
        s: &'a str,
        f: fn(u32, u32, u32, u32) -> Instruction<'a>,
    ) -> Option<Instruction<'a>> {
        let mut v: Vec<Instruction> = r
            .captures_iter(s)
            .map(|v| {
...
impl fmt::Display for Instruction<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        write!(
            f,
            "({} - ({}, {}) to ({}, {})",
            self.ins, self.min_x, self.min_y, self.max_x, self.max_y
        )
    }
}
